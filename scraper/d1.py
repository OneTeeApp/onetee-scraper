...
def sync(db, doc: dict) -> dict:
    ...
    existing = {(r["course_slug"], r["teetime"]): r for r in db.execute(
        "SELECT course_slug, teetime, open_spots, price_min, price_max, active"
        " FROM tee_times WHERE substr(teetime,1,10) = ?", [date])}
...
def main() -> int:
    ...
    p.add_argument("cmd", choices=["init", "push", "stats"])

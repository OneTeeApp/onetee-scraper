...
function renderCity(model, x, stamp) {
  ...
  if (pub.length >= 2) {
    ...
    const medAll = withStats.length >= 3 ? withStats.map((o) => o.s.price_med).sort((a, b) => a - b)[Math.floor(withStats.length / 2)] : null;
...
${intro}
${live.length ? `<h2>Open tee times today in ${esc(x.city)}</h2>` : ""}
